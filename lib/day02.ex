defmodule Day02 do
  ...
  def get_prod_of_final_depth_and_hpos() do
    {final_depth, final_hpos} =
      get_directions()
      |> Enum.reduce({0, 0}, fn {dir, step}, {depth, hpos} ->
        case dir do
  ...
  """
  def get_prod_of_final_depth_and_hpos_with_aim() do
    {final_depth, final_hpos, _final_aim} =
      get_directions()
      |> Enum.reduce({0, 0, 0}, fn {dir, step}, {depth, hpos, aim} ->
        case dir do
          "forward" -> {depth + aim * step, hpos + step, aim}
          "down" -> {depth, hpos, aim + step}
          "up" -> {depth, hpos, aim - step}
        end
      end)
  ...
  defp get_directions() do
    ...
    |> Enum.map(fn dir_step_str ->
      ...
      dir = Enum.at(dir_step_list, 0)
      step = Enum.at(dir_step_list, 1) |> String.to_integer()
      {dir, step}
    end)
  end
end
