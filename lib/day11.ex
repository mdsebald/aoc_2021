defmodule Day11 do
  ...
  # Always only 10 rows and 10 columns of octopus energy
  @max_idx 9

  def count_total_flashes() do
    {octopuses, flash_cnt} =
      get_octopuses()
      |> steps(100, 0)

    print_grid(octopuses)
    IO.puts("Flash count: #{flash_cnt}")
  end

  defp steps(octopuses, 0, flash_cnt), do: {octopuses, flash_cnt}

  defp steps(octopuses, steps, flash_cnt) do
    {octopuses, added_flash_count} = step(octopuses)
    steps(octopuses, steps - 1, flash_cnt + added_flash_count)
  end
  ...
  def steps_to_all_flash() do
    {octopuses, step_cnt} =
      get_octopuses()
      |> steps_to_all_flash()

    print_grid(octopuses)
    IO.puts("Steps to all flash: #{step_cnt}")
  end

  defp steps_to_all_flash(octopuses, step_cnt \\ 1) do
    {octopuses, flash_cnt} = step(octopuses)

    if flash_cnt == 100 do
      {octopuses, step_cnt}
    else
      steps_to_all_flash(octopuses, step_cnt + 1)
    end
  end

  # Common functions

  defp get_octopuses(input_file \\ "inputs/day11.txt") do
  ...
  defp assign_coords(octopuses) do
    Enum.reduce(0..@max_idx, %{}, fn row_idx, indexed_octos ->
      row = Enum.at(octopuses, row_idx)

      Enum.reduce(0..@max_idx, indexed_octos, fn col_idx, indexed_octos ->
        Map.put(indexed_octos, {row_idx, col_idx}, Enum.at(row, col_idx))
      end)
    end)
  end
  ...
  defp add_energy(octopuses) do
    Enum.reduce(octopuses, %{}, fn {coords, energy}, octopuses ->
      Map.put(octopuses, coords, energy + 1)
    end)
  end

  defp count_flashes(octopuses, flash_cnt \\ 0) do
    {octopuses, flashed_octos} =
      Enum.reduce(octopuses, {_octopuses = %{}, _flashed_octos = []}, fn {coord, energy},
                                                                         {octopuses,
                                                                          flashed_octos} ->
        if energy > 9 do
          {Map.put(octopuses, coord, 0), [coord | flashed_octos]}
        else
          {Map.put(octopuses, coord, energy), flashed_octos}
        end
      end)

    # Keep checking for more flashes, until there are none
    added_flash_cnt = length(flashed_octos)

    if added_flash_cnt > 0 do
      octopuses
      |> update_adj_octopuses(flashed_octos)
      |> count_flashes(added_flash_cnt + flash_cnt)
    else
      {octopuses, flash_cnt}
    end
  end

  defp update_adj_octopuses(octopuses, flashed_octos) do
    Enum.reduce(flashed_octos, octopuses, fn {row_idx, col_idx}, octopuses ->
      octopuses
      |> update_octopus({row_idx - 1, col_idx - 1})
      |> update_octopus({row_idx - 1, col_idx})
      |> update_octopus({row_idx - 1, col_idx + 1})
      |> update_octopus({row_idx, col_idx - 1})
      |> update_octopus({row_idx, col_idx + 1})
      |> update_octopus({row_idx + 1, col_idx - 1})
      |> update_octopus({row_idx + 1, col_idx})
      |> update_octopus({row_idx + 1, col_idx + 1})
  ...
  defp update_octopus(octopuses, coord) do
    case Map.get(octopuses, coord) do
      # off the grid of octopuses
      nil -> octopuses
      # already flashed
      0 -> octopuses
      # add energy
      energy -> Map.put(octopuses, coord, energy + 1)
    end
  end

  defp print_grid(octopuses) do
    Enum.each(0..@max_idx, fn row_idx ->
      line =
        Enum.reduce(0..@max_idx, [], fn col_idx, line ->
          line ++ [octopuses[{row_idx, col_idx}]]
        end)
