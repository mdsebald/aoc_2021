defmodule Day08 do
  ...
  """
  def count_unique_digits() do
    get_displays()
    |> Enum.reduce(_unique_count = 0, fn {_patterns, display}, unique_count ->
      unique_count + do_count_unique_digits(display)
    end)
  end

  defp do_count_unique_digits(display) do
    Enum.reduce(display, _count = 0, fn digit, count ->
  ...
  def add_all_values() do
    get_displays()
    |> sort_displays()
    |> Enum.map(&decode_patterns_1/1)
    |> Enum.map(&decode_patterns_2/1)
    |> Enum.map(&decode_patterns_3/1)
    |> Enum.map(&swap_display_pattern/1)
    |> Enum.map(&decode_number/1)
    |> Enum.reduce(_total = 0, fn number, total -> total + number end)
  end

  defp sort_displays(displays) do
    Enum.map(displays, fn {patterns, display} ->
      {Enum.map(patterns, &list_and_sort/1), Enum.map(display, &list_and_sort/1)}
    end)
  end

  defp list_and_sort(string) do
    string
    |> String.to_charlist()
    |> Enum.sort()
  end
  ...
  defp decode_patterns_1({patterns, display}) do
    {display_pattern, rem_patterns} =
      Enum.reduce(patterns, {_display_pattern = %{}, _rem_patterns = []}, fn pattern, {display_pattern, rem_patterns} ->
        case length(pattern) do
          2 ->
            # 2 segments is "1"
            {Map.put(display_pattern, 1, pattern), rem_patterns}

          3 ->
            # 3 segments is "7"
            {Map.put(display_pattern, 7, pattern), rem_patterns}

          4 ->
            # 4 segments is "4"
            {Map.put(display_pattern, 4, pattern), rem_patterns}

          7 ->
            # 7 segments is "8"
            {Map.put(display_pattern, 8, pattern), rem_patterns}

          _ ->
            # Decode remaining patterns in round 2
            {display_pattern, [pattern | rem_patterns]}
        end
      end)

    {display_pattern, rem_patterns, display}
  end

  # Find patterns for "3" and "6"
  defp decode_patterns_2({display_pattern, patterns, display}) do
    {display_pattern, top_right, rem_patterns} =
      Enum.reduce(patterns, {display_pattern, _top_right = [], _rem_patterns = []}, fn pattern,
                                                             {display_pattern, top_right,
                                                              rem_patterns} ->
        case length(pattern) do
          5 ->
            if contains?(pattern, display_pattern[7]) do
              # 5 segments and contains the segments for "7" is "3"
              {Map.put(display_pattern, 3, pattern), top_right, rem_patterns}
            else
              # Decode remaining 5 segment patterns in round 3
              {display_pattern, top_right, [pattern | rem_patterns]}
            end

          6 ->
            if !contains?(pattern, display_pattern[1]) do
              # The "1" segment missing from the "6" is the top right segment
              # Save this for round 3
              r1 = [Enum.at(display_pattern[1], 0)]
              r2 = [Enum.at(display_pattern[1], 1)]
              top_right =
                if !contains?(pattern, r1) do
                  r1
                else
                  r2
                end

              # 6 segments and does not contain top right segment is "6"
              {Map.put(display_pattern, 6, pattern), top_right, rem_patterns}
            else
              # Decode remaining 6 segment patterns in round 3
              {display_pattern, top_right, [pattern | rem_patterns]}
            end
        end
      end)

    {display_pattern, top_right, rem_patterns, display}
  end

  # Find patterns for "2", "5", "9", and "0"
  defp decode_patterns_3({display_pattern, top_right, patterns, display}) do
    display_pattern =
      Enum.reduce(patterns, display_pattern, fn pattern, display_pattern ->
        case length(pattern) do
          5 ->
            if contains?(pattern, top_right) do
              # 5 segments and contains top right segment is "2"
              Map.put(display_pattern, 2, pattern)
            else
              # remaining 5 segment pattern is "5"
              Map.put(display_pattern, 5, pattern)
            end

          6 ->
            if contains?(pattern, display_pattern[3]) do
              # 6 segments and contains the segments for "3" is "9"
              Map.put(display_pattern, 9, pattern)
            else
              # Remaining 6 segment pattern is "0"
              Map.put(display_pattern, 0, pattern)
            end
        end
      end)
  ...
  defp swap_display_pattern({display_pattern, display}) do
    {Map.new(display_pattern, fn {key, val} -> {val, key} end), display}
  end

  defp decode_number({pattern_display, display}) do
    Enum.reduce(display, _number = 0, fn digit_pattern, number ->
      10 * number + pattern_display[digit_pattern]
    end)
  end

  defp contains?(pattern, test) do
    Enum.reduce(test, _result = true, fn element, result ->
      result and Enum.member?(pattern, element)
    end)
  end

  # Common functions

  defp get_displays(input_file \\ "inputs/day08.txt") do
    File.read!(input_file)
    |> String.split("\n")
    |> Enum.map(fn patterns ->
      [raw_patterns, raw_display] = String.split(patterns, "|")
      {String.split(raw_patterns, " ", trim: true), String.split(raw_display, " ", trim: true)}
    end)
  end
end
