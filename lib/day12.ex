defmodule Day12 do
  ...
  defp explore_caves(_caves, "end", visited), do: ["end" | visited]

  defp explore_caves(caves, curr_cave, visited) do
    # IO.puts("Current cave: #{curr_cave}, Caves visited: #{inspect(visited)}")

    caves[curr_cave]
    |> Enum.filter(fn cave -> big_cave?(cave) or !Enum.member?(visited, cave) end)
    |> Enum.map(fn cave ->
      explore_caves(caves, cave, [curr_cave | visited])
    end)
  end
  ...
  def count_paths_2() do
    get_caves()
    |> explore_caves_2("start", [], true)
    |> count_paths()

  end

  defp explore_caves_2(_caves, "end", visited, _small), do: ["end" | visited]

  defp explore_caves_2(_caves, "start", [_|_] = visited, _small), do: visited

  defp explore_caves_2(caves, curr_cave, visited, small) do
    small =
      cond do
        small == false -> false
        small_cave?(curr_cave) and Enum.member?(visited, curr_cave) -> false
        true -> true
      end

    caves[curr_cave]
    |> Enum.filter(fn cave -> small or big_cave?(cave) or !Enum.member?(visited, cave) end)
    |> Enum.map(fn cave ->
      explore_caves_2(caves, cave, [curr_cave | visited], small)
    end)
  end

  # Common functions

  defp get_caves(input_file \\ "inputs/day12.txt") do
    caves =
      File.read!(input_file)
      |> String.split("\n")
      |> Enum.map(&String.split(&1, "-"))

    # Group forward and reversed cave connections and merge
    forward_conns = Enum.group_by(caves, &Enum.at(&1, 0), &Enum.at(&1, 1))
    reversed_conns = Enum.group_by(caves, &Enum.at(&1, 1), &Enum.at(&1, 0))

    Map.merge(forward_conns, reversed_conns, fn _cave, conns_f, conns_r ->
      conns_f ++ conns_r
    end)
  end

  defp count_paths(paths) do
    paths
    |> List.flatten()
    |> Enum.frequencies()
    |> Map.get("end")
  ...
  defp big_cave?(cave), do: String.upcase(cave) == cave
