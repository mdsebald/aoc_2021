defmodule Day07 do
  ...
  def det_horz_pos() do
    ...
    {min_pos, max_pos} = get_min_max_pos(crabs)

    Enum.reduce(min_pos..max_pos, 999_999_999, fn targ_pos, fuel ->
      min(fuel, calc_fuel(crabs, targ_pos))
    end)
  end

  defp calc_fuel(crabs, targ_pos) do
    Enum.reduce(crabs, 0, fn {pos, qty}, fuel ->
      fuel + abs(targ_pos - pos) * qty
    end)
  end
  ...
  def det_horz_pos_2() do
    ...
    pos_fuel = pre_compute_fuel(max_pos)

    Enum.reduce(min_pos..max_pos, 999_999_999, fn targ_pos, fuel ->
      Enum.map_reduce(0..max_pos, _prev_sum = 0, fn pos, prev_sum ->
        next_sum = pos + prev_sum
        {next_sum, next_sum}
      end)

      min(fuel, calc_fuel_2(crabs, targ_pos, pos_fuel))
    end)
  end

  defp calc_fuel_2(crabs, targ_pos, pos_fuel) do
    Enum.reduce(crabs, 0, fn {pos, qty}, fuel ->
      fuel + Enum.at(pos_fuel, abs(targ_pos - pos)) * qty
    end)
  end

  defp pre_compute_fuel(max_pos) do
    {pos_fuel, _last_pos} =
      Enum.map_reduce(0..max_pos, _last_sum = 0, fn pos, last_sum ->
        last_sum = pos + last_sum
        {last_sum, last_sum}
      end)
  ...
  defp get_min_max_pos(crabs) do
    Enum.reduce(crabs, {999_999_999, 0}, fn {pos, _qty}, {min, max} ->
