defmodule Day06 do
  ...
  defp do_fish_cycle(fishes, 0), do: fishes

  defp do_fish_cycle(fishes, days) do
    {fishes, new_fish_cnt} =
      Enum.map_reduce(fishes, _new_fish_cnt = 0, fn fish, new_fish_cnt ->
        case fish do
          0 -> {6, new_fish_cnt + 1}
          fish -> {fish - 1, new_fish_cnt}
        end
      end)

    fishes =
      if new_fish_cnt > 0 do
        fishes ++ Enum.map(1..new_fish_cnt, fn _index -> 8 end)
      else
        fishes
      end

    do_fish_cycle(fishes, days - 1)
  end
  ...
  def count_fish_mapped() do
    get_fish()
    |> Enum.frequencies()
    |> count_using_map(256)
  end

  defp count_using_map(fishes, 0) do
    # IO.puts("Fishes: #{inspect(fishes)}")
    Enum.reduce(fishes, _total = 0, fn {_age, count}, total ->
      total + count
    end)
  end

  defp count_using_map(fishes, days) do
    # IO.puts("Fishes: #{inspect(fishes)}")
    Enum.reduce(fishes, %{}, fn {age, count}, acc ->
      # IO.puts("Day: #{days}, Age: #{age}, Count: #{count}, Accum: #{inspect(acc)}")
      case age do
        0 ->
          acc = Map.put(acc, 6, count)
          Map.put(acc, 8, count)
        7 ->
          # There could already be fish age = 6, in the new map
          count = count + Map.get(acc, 6, 0)
          Map.put(acc, 6, count)
        age ->
          Map.put(acc, age - 1, count)
      end
    end)
    |> count_using_map(days - 1)
  end
